...
const file: Record<string, Function> = {};
const folder: Record<string, Function> = {};
...
file.fullPublicPath = (trimmedFilePath: string): string => {
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
 ...
 */
file.create = async (dir: string, fileName: string, content: string): Promise<[boolean, string | Error]> => {
...
file.readPublic = async (trimmedFilePath: string) => {
    try {
...
file.readPublicBinary = async (trimmedFilePath: string) => {
    try {
        const filePath = file.fullPublicPath(trimmedFilePath);
 ...
 */
file.update = async (dir: string, fileName: string, content: object): Promise<[boolean, string | Error]> => {
    let fileDescriptor = null;
    try {
        const filePath = file.fullPath(dir, fileName);
